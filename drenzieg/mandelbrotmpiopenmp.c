/* mandelbrotmpiopenmp.c */
/*this program is basically a combination of static mpi and openmp implementation
...
#pragma omp for schedule (static,1)  /*could be runtime so its schedule is decided on start*/
#pragma omp parallel for private(j) 
if(my_rank == 0)
{
	for(i=0; i < X_RESN; i++)
	{
		MPI_Recv(message, Y_RESN+1, MPI_LONG, i+1, 1, MPI_COMM_WORLD, &status);
